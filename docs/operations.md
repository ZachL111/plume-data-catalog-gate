...
`plume-data-catalog-gate` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies catalog behavior through log and snapshot fixtures, with replay consistency checks and synthetic fixtures only.
...
The score is deterministic. The current threshold is 160. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 5.
